...
class RuleRegistry {
  ...
  static final Map<RuleId, LintRule Function(Iterable<Glob>)> _rules = {
    RuleId.preferHttpsOverHttp: (excludes) => PreferHttpsOverHttp(excludes: excludes),
    RuleId.avoidHardcodedUrls: (excludes) => AvoidHardcodedUrls(excludes: excludes),
    RuleId.avoidWeakHashing: (excludes) => AvoidWeakHashing(excludes: excludes),
    RuleId.preferSecureRandom: (excludes) => PreferSecureRandom(excludes: excludes),
    RuleId.avoidHardcodedSecrets: (excludes) => AvoidHardcodedSecrets(excludes: excludes),
  };
